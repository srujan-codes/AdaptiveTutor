...
{
  "next_topic": "Specific topic name",
  "reasoning": "2-3 sentences referencing student data.",
  "estimated_difficulty": "beginner|intermediate|advanced",
  "related_topics": ["Alt 1", "Alt 2", "Alt 3"]
}

RULES:
1. Only valid JSON. 2. Topic must be specific (not just "Python"). 3. Exactly 3 related topics.
4. Difficulty matches student level + performance. 5. New users → foundational popular topic.
6. Never jump 2 difficulty levels. 7. Suggest branching out if student studies only one subject.
8. Return ONLY a valid JSON object, no markdown, no backticks, no newlines inside strings, no explanatory text."""
...
class StrategyPlannerAgent(BaseAgent):
    ...
    def parse_response(self, data: dict) -> dict:
        ...
        recommended_topic = data.get("next_topic") or data.get("recommended_topic", "Introduction to Programming")
        reason = data.get("reasoning") or data.get("reason", "This is a great foundational topic to start with.")
        suggested_difficulty = (data.get("estimated_difficulty") or data.get("suggested_difficulty", "beginner")).lower().strip()
        alternatives = data.get("related_topics") or data.get("alternatives", [])
        ...
        return {
            "recommended_topic": str(recommended_topic),
            "next_topic": str(recommended_topic),
            "reason": str(reason),
            "reasoning": str(reason),
            "suggested_difficulty": suggested_difficulty,
            "estimated_difficulty": suggested_difficulty,
            "alternatives": alternatives,
            "related_topics": alternatives,
